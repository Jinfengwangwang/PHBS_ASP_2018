...
class ModelHagan:
    ...
    def calibrate3(self, price_or_vol3, strike3, spot, texp=None, cp_sign=1, setval=False, is_vol=True):
        ...
        if(is_vol):
            ...
            vol3 = [self.bsm_model.impvol(price_or_vol3[i], strike3[i], spot, texp, cp_sign=cp_sign) for i in range(3)]

        def func(x):
            # Since sigma >=0, alpha >=0, -1<=rho<=1
            # set constraints to the parameters
            sigma = np.sqrt(x[0]**2)
            alpha = np.sqrt(x[1]**2)
            rho = 2*x[2]/(1+x[2]**2)
            
            bsm_vol1 = bsm_vol(strike3[0], spot, texp, sigma, alpha=alpha, rho=rho) - vol3[0]
            bsm_vol2 = bsm_vol(strike3[1], spot, texp, sigma, alpha=alpha, rho=rho) - vol3[1]
            bsm_vol3 = bsm_vol(strike3[2], spot, texp, sigma, alpha=alpha, rho=rho) - vol3[2]
...
class ModelBsmMC:
    ...
    def bsm_vol(self, strike, spot, texp=None, sigma=None, cp_sign=1, n_step=50, n_sample=100):
        ...
        use bsm_model
        '''
        price_mc = self.price(strike, spot, texp, sigma, cp_sign, n_step, n_sample)
        
        def iv_func(sigma):
            return self.bsm_model.price(strike, spot, texp, sigma, cp_sign) - price_mc
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1, n_step=50, n_sample=100):
        ...
        delta_t = self.texp / n_step
        znorm_m = np.random.normal(size=(strike.size, n_step, 2, n_sample))
...
class ModelNormalMC:
    ...
    def norm_vol(self, strike, spot, texp=None, sigma=None, cp_sign=1, n_step=50, n_sample=100):
        ...
        def iv_func(sigma):
            return self.normal_model.price(strike, spot, texp, sigma, cp_sign) - price_mc
...
class ModelBsmCondMC:
    ...
    def bsm_vol(self, strike, spot, texp=None, sigma=None, cp_sign=1, n_step=50, n_sample=100):
        ...
        def iv_func(sigma):
            return self.bsm_model.price(strike, spot, texp, sigma, cp_sign) - price_cmc
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp_sign=1, n_step=50, n_sample=100):
        ...
        for j in range(n_sample):  
            ...
            price_array[:, j] = self.bsm_model.price(strike, new_spot, texp, new_sigma, cp_sign)  
